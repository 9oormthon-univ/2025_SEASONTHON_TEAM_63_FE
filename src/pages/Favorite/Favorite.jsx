...
function Favorite() {
  ...
  const setStoreSubscription = useFavoriteStore(
    (state) => state.setStoreSubscription
  );
  const initializeSubscriptions = useFavoriteStore(
    (state) => state.initializeSubscriptions
  );
    ...
    const fetchInitialFavoriteShops = async () => {
      ...
      try {
        ...
        if (response.data && response.data.success) {
          ...
          setInitialFavorites(favoriteShops);

          // [핵심 수정] 전역 상태를 일괄로 초기화 (더 효율적)
          const storeIds = favoriteShops.map((shop) => shop.id);
          initializeSubscriptions(storeIds);

          console.log('찜한 가게 목록 로드 완료:', favoriteShops.length, '개');
          console.log('전역 상태 초기화 완료:', storeIds);
        } else {
          throw new Error(
            response.data.message || '찜한 가게 목록을 가져오는데 실패했습니다.'
          );
        }
      } catch (err) {
  ...
  const displayedShops = initialFavorites.filter((shop) =>
    subscribedStores.has(String(shop.id))
  );
      ...
      {displayedShops.length > 0 ? (
        ...
        <div className="Favorite-empty-container">
          <img
            ...
            alt="찜한 가게 없음"
            className="Favorite-empty-image"
          />
